...
export default class News extends Component {
  ...
  async loadNews(){
    ...
    const prevArticleLength = parsedData.articles.length
    parsedData.articles = parsedData.articles.filter(article => {
      return article.source.name !== "[Removed]";
    });
    const newArticleLength = parsedData.articles.length
    this.setState({removedArticles: prevArticleLength - newArticleLength})
    parsedData.totalResults = parsedData.totalResults - (prevArticleLength - newArticleLength)
  ...
  fetchMoreData = async () => {
    ...
    let parsedData = await data.json();
    const prevArticleLength = parsedData.articles.length
    console.log("prev="+prevArticleLength)
    parsedData.articles = parsedData.articles.filter(article => {
      return article.source.name !== "[Removed]";
    });
    const newArticleLength = parsedData.articles.length
    parsedData.totalResults = parsedData.totalResults - (this.state.removedArticles + (prevArticleLength - newArticleLength))
    this.setState({removedArticles :this.state.removedArticles + (prevArticleLength - newArticleLength)})
    console.log("new="+newArticleLength)
    console.log(parsedData);
    this.setState({
                ...
                {this.state.articles.map((element)=>{
                  return <div className="col-md-4 my-3" key={element.url}>
                    <NewsItem
                      ...
                      imgUrl={element.urlToImage?element.urlToImage:process.env.PUBLIC_URL + '/OPInsightsPreview.jpg'} 
                      newsUrl ={element.url}
